# E2E tool check (one-test-per-tool). Round-trips pcg-generate through the live MCP server.
...
# game-thread world access are both exercised without seeding an actor (a real generate needs an
# actor with a PCG component, validated by the Automation spec + a live smoke).
@{
    Tool        = "pcg-generate"
    System      = $false
